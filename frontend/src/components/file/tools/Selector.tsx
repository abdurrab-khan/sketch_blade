...
const Selector: React.FC<SelectorProps> = ({ stageRef, trRef, isHovered, setIsHovered }) => {
  ...
  const handleShapeSelectionBySelector = useCallback(
    (tr: Konva.Transformer, stage: Stage, selector: Konva.Rect) => {
      ...
      if (selected.length > 0 && selected.length !== selectedShapesId?._id.length) {
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (activeTool === "cursor" && selectorRef?.current?.visible()) {
